 ...
 * @LastEditTime: 2025-04-02 22:39:11
 * @Description: 文件描述
      ...
      <div class="travel-item" v-for="(item, index) in list" :key="item.id">
        <div class="image-container">
          ...
          <div class="image-controls">
            ...
            <button class="zoom-in" @click="zoomIn(item, index)">
              放大10%
            </button>
            <button class="zoom-out" @click="zoomOut(item, index)">
              缩小10%
            </button>
            <button class="move-left" @click="moveLeft(item, index)">
              左移10px
            </button>
            <button class="move-right" @click="moveRight(item, index)">
              右移10px
            </button>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    const rotateLeft = (item,index) => {
      const {rotation,scale} = item
      list.value[index] = { ...list.value[index], rotation: rotation - 10, scale: scale }
    }
    const rotateRight = (item,index) => {
      const {rotation,scale} = item
      list.value[index] = { ...list.value[index], rotation: rotation + 10, scale: scale }
    }
    const zoomIn = (item,index) => {
      const {rotation,scale} = item
      list.value[index] = { ...list.value[index], rotation: rotation, scale: scale * 1.1 }
    }
    const zoomOut = (item, index) => {
      const { rotation, scale } = item
      const newScale = Math.max(0.5, scale / 1.1)
      list.value[index] = { ...list.value[index], rotation, scale: newScale }
    }
    const moveLeft = (item, index) => {
      const { rotation, scale } = item
      list.value[index] = { ...list.value[index], rotation, scale: scale, x: item.x - 10 }
    }
    const moveRight = (item, index) => {
      const { rotation, scale } = item
      list.value[index] = { ...list.value[index], rotation, scale: scale, x: item.x + 10 }
    }
